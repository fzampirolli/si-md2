...
def process_cell(source, key_to_num: dict, elem_map: dict, bib: dict) -> list:
    ...
    # Restaura \@palavra -> @palavra (@ literal no texto do aluno)
...
def clean_notebook(notebook: dict) -> dict:
    ...
    for cell in notebook.get("cells", []):
        ...
        # Secao de referencias: extrai parágrafo introdutório e descarta o restante
...
def build_reference_list(citations: list, bib: dict) -> tuple:
    ...
    intro_paragraph: parágrafo introdutório já com @key resolvidos.
    """
    ...
    def get_sort_key(key):
        ...
        if author_raw:
            ...
            surname = first_author.split(",")[0] if "," in first_author \
...
def process_notebook(nb_path: Path, bib: dict, out_path: Path) -> list:
    ...
    # Limpeza antes de processar (extrai parágrafo introdutório da seção de refs)
    notebook = clean_notebook(notebook)

    # Recupera o parágrafo introdutório extraído por clean_notebook
    intro_raw = notebook.pop("_ref_intro", "")

    # Resolve @citações no parágrafo introdutório (mesmo pipeline de process_cell)
    # Usamos key_to_num vazio por ora; será recalculado após build_reference_list
    ref_markdown, key_to_num = build_reference_list(citations, bib,
                                                     intro_paragraph=intro_raw)

    # Agora resolve as @citações no parágrafo com o key_to_num correto
    if intro_raw:
        intro_resolved = source_to_str(
            process_cell(str_to_source(intro_raw), key_to_num, elem_map, bib)
        )
        ref_markdown, key_to_num = build_reference_list(citations, bib,
